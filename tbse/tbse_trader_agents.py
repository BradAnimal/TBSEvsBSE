...
class TraderGA(Trader):
    def __init__(self, ttype, tid, balance, time, genes=None):
        super().__init__(ttype, tid, balance, time)
        ...
        if genes:
            ...
            self.genes = np.random.uniform(GENE_BOUNDS["low"], GENE_BOUNDS["high"], size=NUM_GENES)
    ...
    def get_order(self, time, countdown, lob):
        ...
        order = Order(self.tid, order.otype, quotePrice, self.orders[coid].qty, time, order.coid, order.toid)
    ...
    def bookkeep(self, trade, order, vrbs, time):
        ...
        hrs = int(mins//60)
        mins = mins - 60 * hrs
        outstr = f't={time} ({hrs}h{mins}m{secs}s) {self.tid} ({self.ttype}) bookkeep: orders='
        for i in self.orders:
            outstr = outstr + str(i)
        self.blotter.append(trade)
        transPrice = trade["price"]
        ...
        if coid:
            ...
            otype = self.last_quote.otype if self.last_quote else "Bid"

        profit = 0.0
        if otype == "Bid":
            profit = limit - transPrice
        else:
            profit = transPrice - limit
        
        self.balance += profit
        self.sessionProfit += profit
        self.n_trades += 1
        self.profitpertime = self.balance / (time - self.birth_time)
...
class TraderRL(Trader):
    def __init__(self, ttype, tid, balance, time, snapshot=None, minVal=75, maxVal=200):
        super().__init__(ttype, tid, balance, time)
        ...
        if snapshot:
            ...
            self.stateSize = snapshot["stateSize"]
            self.actionSize = snapshot["actionSize"]
            self.qTable = dict(snapshot["qTable"])
            self.epsilon = max(snapshot["epsilon"] * 0.995, 0.006)
    ...
    def get_order(self, time, countdown, lob):
        ...
        coid = max(self.orders.keys())
        custOrder = self.orders[coid]

        self.lastCountdown = countdown
        state = self.getState(lob, countdown)
        action = self.chooseAction(state)
        price = self.actionToPrice(action, custOrder)
        ...
        order = Order(self.tid, custOrder.otype, price, custOrder.qty, time, custOrder.coid, custOrder.toid)
        self.last_quote = order
        return order
    ...
    def respond(self, time, lob, trade, vrbs):
        ...
        newState = self.getState(lob, self.lastCountdown)
        self.updateQTable(self.lastState, self.lastAction, reward, newState)
        self.epsilon = max(self.epsilon * 0.995, 0.006)
    ...
    def bookkeep(self, trade, order, vrbs, time):
        ...
        self.n_trades += 1
        self.profitpertime = self.balance / (time - self.birth_time) if (time - self.birth_time) else profit
        ...
        if vrbs:
            print(f"{self.tid} RL profit={profit} balance={self.balance} profit/time={str(self.profitpertime)}")
        self.del_order(coid)
    ...
    def actionToPrice(self, action, order):
        limit = order.price
        otype = order.otype
    ...
    def computeReward(self, trade):
        if trade and (trade["party1"] == self.tid or trade["party2"] == self.tid):
            if self.orders:
                ...
                limit = self.orders[coid].price
                otype = self.orders[coid].otype
            elif self.last_quote:
                limit = self.last_quote.price
                otype = self.last_quote.otype
            else:
                return 0.0
            ...
            if otype == "Bid":
                ...
                profit =  trade["price"] - limit
            timeBonus = 1 - self.lastCountdown
            return (profit * (1+timeBonus))
        return (-0.1 * (1+self.lastCountdown))
    ...
    def bin(self, value, numBins):
        ...
        return int((value-self.minVal) / binSize)
    
    def binCount(self, value, numBins):
